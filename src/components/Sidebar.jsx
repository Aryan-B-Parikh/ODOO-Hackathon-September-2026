...
const NavItem = ({ to, icon, label, badge }) => (
  ...
  >
    ...
    {badge > 0 && (
      <span className="ml-auto min-w-[18px] h-[18px] px-1 bg-error text-on-error text-[9px] font-bold rounded-full flex items-center justify-center animate-pulse">
        {badge > 9 ? '9+' : badge}
      </span>
    )}
  </NavLink>
...
export default function Sidebar() {
  ...
  return (
    <aside className="fixed left-0 top-0 h-full w-sidebar-width bg-surface dark:bg-[#111827] border-r border-outline-variant dark:border-[#1E293B] shadow-sm z-50 flex flex-col py-6 transition-colors duration-300">
      {/* Logo */}
      ...
      <nav className="flex-1 px-3 space-y-0.5 overflow-y-auto custom-scrollbar">
        ...
        {user.role === 'Admin' && (
          <NavItem to="/organization" icon="corporate_fare" label="Organization" />
        )}
        ...
        <NavItem to="/profile" icon="person" label="My Profile" />
      </nav>
        ...
        <div className="flex items-center gap-3 px-3 py-2 rounded-xl hover:bg-surface-container-high dark:hover:bg-surface-container transition-colors">
          <div className="shrink-0">
            {user.avatar ? (
              ...
              <div className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold ${user.avatarBg || 'bg-primary text-on-primary'}`}>
                {user.initials || user.name?.split(' ').map(n => n[0]).join('') || 'U'}
              </div>
            )}
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-on-surface truncate leading-tight">{user.name}</p>
            <p className="text-[10px] text-on-surface-variant truncate">{user.role}</p>
          </div>
          <button
            onClick={logout}
            className="p-1.5 rounded-lg hover:bg-error-container/20 text-on-surface-variant hover:text-error transition-all shrink-0"
            title="Sign Out"
          >
